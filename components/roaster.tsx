...
export function Roaster() {
  ...
  useEffect(() => {
    if (roast && roastRef.current) {
      ...
      text.innerHTML = roast.split('').map(char => 
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-gray-100">
      <Card className="w-full max-w-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-left">GitHub Profile Roaster</CardTitle>
          <CardDescription className="text-left">Paste your GitHub profile URL and get roasted!</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {loading && (
            <div className="mt-6 p-4 bg-gray-100 rounded-md">
              <div className="flex items-center space-x-2 overflow-hidden">
                ...
                <p className="text-gray-700 text-sm whitespace-nowrap overflow-hidden text-ellipsis">{loadingRoast}</p>
              </div>
            </div>
          )}
          {roast && !loading && (
